...
module IRCanonicity where
open import Lib

module Canonicity (i : Level) (j : Level) (O : Set j) (Oᵒ : O → Set j) where

  import PlainIR as IR
  open import PlainIR hiding (Sig)
  import IndexedIR
...
-- We provide examples separately, because we need to instantiate some parameters to the canonicity
-- construction.
----------------------------------------------------------------------------------------------------
...
module Examples where
  ...
  module Example-4-4 where
    ...
    module C = Canonicity zero (suc zero) Set
