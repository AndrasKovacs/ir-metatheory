...
module DeriveIndexed where
open import Lib

module Derivation {i j k : Level}(I : Set k)(O : I → Set j) where
...
-- Example construction of Code and Vec as IIR types, constructed from IR types
----------------------------------------------------------------------------------------------------
...
module DeriveIndexedExamples where
  ...
  module VecExample {i}(A : Set i) where
    ...
    open Derivation {i} N.ℕ (const (⊤{zero}))
  ...
  module CodeExample where
    ...
    module IIR = Derivation {zero} (⊤{zero}) (const Set)
