...
\begin{frame}{IR implementation \& syntactic metatheory}

IR has been supported in Agda for 15+ years. Also available in Idris-es.
\vspace{1em}

\emph{Informally} the implementation is quite straightforward.
\vspace{1em}
\pause
...
Quotient inductive-inductive definition with these sorts:\footnote{We omit universe levels here and in the rest of the talk}
\begin{alignat*}{3}
  & \Con &&: \Set\\
  & \Ty  &&: \Con \to \Set\\
  & \Sub &&: \Con \to \Con \to \Set\\
...
We have
\begin{itemize}
  \item An explicit substitution calculus, given as a category with family.
  \item Countable universes in Russell style.
...
\textbf{Artin gluing} is an established proof-relevant version of \textbf{logical predicate interpretation} which
...
In the syntax, assume $\Nat : \Ty\,\Gamma$ with $\zero : \Tm\,\Gamma\,\Nat$ and $\suc : \Tm\,\Gamma\,\Nat \to \Tm\,\Gamma\,\Nat$.
\vspace{1em}
\pause

Closed canonical terms are specified by a \alert{metatheoretic indexed inductive type}.
\begin{alignat*}{3}
...
\end{alignat*}
\textbf{Example:} the signature for $\Code$:
\begin{alignat*}{4}
  & \ms{SigCode} : \Sig\,\Set\\
...
\begin{frame}{General canonicity}

Somewhat technical. I try to give computational intuition.
\pause

\begin{itemize}
\item The computational content of $\blank^\w$ is \alert{type-directed closed evaluation}.
\item Canonicity witnesses correspond to \alert{runtime values}.
\end{itemize}
\vspace{1em}
\pause
...
\begin{alignat*}{3}
  ...
  & \text{\emph{is proved canonical by}}\quad &&\text{ETT with $\omega$ universes with IR + $\Set_\omega$ and $\Set_{\omega+1}$}\\
  & \text{\emph{is proved consistent by}}\quad &&\text{ZF with $\omega$ Mahlo cardinals + two inaccessible cardinals above}
\end{alignat*}
