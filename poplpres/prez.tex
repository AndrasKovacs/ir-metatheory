...
$\Code$ and $\El$ yield a function $\ms{F} : \ms{FamSet} \to \ms{FamSet}$.
\vspace{1em}
...
\begin{itemize}
  \item Consistency \cite{first-class-univ}
  \item Canonicity \cite{DBLP:journals/corr/abs-2502-20485}
  \item Here at POPL: normalization \cite{10.1145/3776645}
\end{itemize}
\vspace{1em}
...
\begin{frame}{Implementation \& Metatheory}

IR has been supported in Agda for 15+ years. Also supported in Idris-es.
\vspace{1em}

Intuitively, there is \emph{nothing special} about the implementation.
\vspace{1em}
\pause
...
We show \textbf{canonicity} in the current work.
...
\begin{alignat*}{3}
  ...
  & \Ty  &&: \Con \to \Nat \to \Set\\
  & \Sub &&: \Con \to \Con \to \Set\\
  & \Tm  &&: (\Gamma : \Con)\{i : \Nat\} \to \Ty\,\Gamma\,i \to \Set
\end{alignat*}

We have
\begin{itemize}
  \item An explicit substitution calculus, given as a category with families.
  \item Russell-style universes, as $\U : (i : \Nat) \to \Ty\,\Gamma\,(i + 1)$ such that
    $\Tm\,\Gamma\,(\U\,i) = \Ty\,\Gamma\,i$.
...
We define a family of functions $\blank^\circ$ from the syntax by induction.
\begin{alignat*}{3}
...
\textbf{Example:} the canonicity predicate for the IR type $\Code$ is an \alert{indexed IR type} in the metatheory.
\pause
\begin{alignat*}{4}
