...
interpretation, which are the CwF, universes and functions. First, we revisit the specification of
the induction motives, now using $\G$:
\begin{alignat*}{3}
...
\end{alignat*}
Hence, we define an inductive type in the metatheory that is parameterized by $O :
\Tm\,\emptycon\,U_i$ and $O^\w : \Tm\,\emptycon\,O \to \Set_i$ and indexed over
$\Tm\,\emptycon\,(\Sig_i\,O)$. We name this inductive type $\Sig^\w$, which risks some confusion,
but we shall take the risk.
\begin{alignat*}{4}
  &\rlap{$\data\,\Sig^\w  : \Tm\,\emptycon\,(\Sig_i\,O) \to \Set_{\max(i+1,\,j)}$}\\
  &\quad \iota^\w  &&:\,&&\{o &&: \Tm\,\emptycon\,O\}(o^\w : O^\w\,o) \to \Sig^\w\,(\iota\,o)\\
  &\quad \sigma^\w &&:\,&&\{A &&: \Tm\,\emptycon\,\U_i\}(A^\w : \Tm\,\emptycon\,A \to \Set_i)\\
  &               && &&\{S &&: \Tm\,\emptycon\,(A \to \Sig_i\,O)\}\\
  &               && && (S^\w &&: \{a : \Tm\,\emptycon\,A\} \to A^\w\,a \to \Sig^\w\,(S\,a))\\
  &               && && \to &&\,\Sig^\w\,(\sigma\,A\,S)\\
  &\quad \delta^\w &&:\,&&\{A &&: \Tm\,\emptycon\,\U_i\}(A^\w : \Tm\,\emptycon\,A \to \Set_i)\\
  &               &&   && \{S &&: \Tm\,\emptycon\,((A \to O) \to \Sig_i\,O)\}\\
  &               &&   && (S^\w &&: \{f : \Tm\,\emptycon\,(A \to O)\} \to (\{a : \Tm\,\emptycon\,A\} \to A^\w\,a \to O^\w\,(f\,a)) \to \Sig^\w\,(S\,f))\\
  &\quad          &&   && \to &&\,\Sig^\w\,(\delta\,S\,f)
\end{alignat*}
$\Sig^\w\,t$ witnesses that $t$ is canonical and only contains inductively canonical data.
...
%% ((O : \U_j) \to \U_{\max(i+1,\,j)})^\w\,\gamma^\w\,\Sig_i\\
