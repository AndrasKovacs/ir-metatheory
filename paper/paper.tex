...
\end{alignat*}
Here we quantify over an $S$ that is possibly distinct from $S^*$. Now, we can show that
...
be shorty required for rearranging some transports. Next, we show that the two $\blank_1$ operations
are the same, modulo the previous equivalence, again by induction on IIR signatures.
...
\end{alignat*}
We adjust $f$ to obtain the next argument to $\elim_\IR$. $f$ takes $(S^*)_\IH\,P\,x$ as input,
