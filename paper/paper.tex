...
\paragraph{Interpretation of $\intro$ and $\Elintro$.}
For this, we need to show an equivalence between two representations of $\IR^\w$'s data, somewhat
similarly to as in Section \ref{TODO}. First, we define the predicate interpretations of $\blank_0$
and $\blank_1$:
\begin{alignat*}{3}
  & \blank_0 : \Sig^\w\,S \to \Tm\,(S_0\,(\IR\,S^*)\,\El) \to \Set_i\\
  & \blank_1 : (S^\w : \Sig^\w\,S)\{t : \Tm\,(S_0\,(\IR\,S^*)\,\El)\} \to (S^\w)_0\,t \to O^\w\,(S_1\,t)
\end{alignat*}
Second, we define
\begin{alignat*}{3}
  &\blank_{\floord{0}} : (S^\w : \Sig^\w\,S) \to \Path\,S^\w \to \Tm\,(\IR\,S^*) \to \Set_i\\
  &(S^\w)_{\floord{0}}\,p\,t := (t' : \Tm\,(S_0\,(\IR\,S^*)\,\El)) \times (\intro\,(\push_0\,p\,t') = t) \times (S^\w)_0\,t'.
\end{alignat*}
Next, we show the following equivalence by induction on $S^\w$:
\[  (S^\w : \Sig^\w\,S)(p : \Path\,S^\w)\{t : \Tm\,(\IR\,S^*)\} \to (S^\w)_{\floord{0}}\,p\,t \simeq (\floord{S^\w}\,p)_0\,\IR^\w\,\El^\w\,t \]
We write $\ora{(S^\w)_0}\,p$ for the map with type $(S^\w)_{\floord{0}}\,p\,t \to (\floord{S^\w}\,p)_0\,\IR^\w\,\El^\w\,t$ and
$\ola{(S^\w)_0}\,p$ for its inverse. Now we can interpret $\intro$.
\begin{alignat*}{4}
  & \intro^\w : \{t : \Tm\,((S^*)_0\,(\IR\,S^*)\,\El)\} \to (\Ssw)_0\,t \to \IR^\w\,(\intro\,t)\\
...
%% Now, we have two types that should mean the same thing, $\PSbe_0\,\IR^\w\,\El^\w\,t$ and



%% Now, we have two types that specify the data contained in $\IR^\w\,t$, and we have to prove them equivalent to
%% interpret the other IR rules.

%% The \textbf{first type} is  $\PSbe_0\,\IR^\w\,\El^\w\,t$ which we directly get from the IIR rules.

%% For the \textbf{second type} we need some auxilliary definitions. We define predicate interpretations of $\blank_0$ and $\blank_1$:
%% \begin{alignat*}{3}
%%   & \blank_0 : \Sig^\w\,S \to \Tm\,(S_0\,(\IR\,S^*)\,\El) \to \Set_i\\
%%   & \blank_1 : (S^\w : \Sig^\w\,S)\{t : \Tm\,(S_0\,(\IR\,S^*)\,\El)\} \to (S^\w)_0\,t \to O^\w\,(S_1\,t)
%% \end{alignat*}
%% We define the other type as follows.
%% \begin{alignat*}{3}
%%   &\floord{\blank} : (S^\w : \Sig^\w\,S) \to \Path\,S^\w \to \Tm\,(\IR\,S^*) \to \Set_i\\
%%   &\floord{S^\w}\,p\,t := (t' : \Tm\,(\IR\,S^*)) \times (\intro\,(\push_0\,p\,t') = t) \times (S^\w)_0\,t'
%% \end{alignat*}



%% However, to interpret the other rules, we have to again show an equivalence between two descriptions of the data
%% that is contained in $\IR^\w\,t$. First, we have $\PSbe_0\,\IR^\w\,\El^\w\,t$. The second representation
