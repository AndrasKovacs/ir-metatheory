...
\end{alignat*}
This $\U$, unlike the ambient $\Set_0$ universe, supports an induction principle and can be used to
...
The IR specification is parameterized over the following data:
\begin{itemize}
...
\end{itemize}
IR signatures are specified by the following inductive type:
\begin{alignat*}{4}
  &\data\, \Sig : \U_{\max(i+1,\,o)}\\
...
\begin{example}
The Agda example on Figure \cite{TODO} can be specified as follows. We assume a
...
%% , which we denote as $\le$ and $\lo$.



%% Recall the IR example from Figure \ref{TODO}.




%% : there, we defined a type which resides in $\Set_0$
%% and whose recursive component $\El$ returns in $\Set_0$ as well. These two universe


%% The specification of IR types is parameterized over three pieces of data:

%% \begin{itemize}
%% \item Universe levels $l_e$ and $l_o$.
%% \item A type $O : \U_{l_o}$.
%% \end{itemize}
