...
of the following.
\begin{enumerate}
\item A type of signatures. Each signature describes an IR type. Since signatures are first-class internal values,
      we also internally define some \emph{semantics of signatures}, interpreting them as endofunctors on slice categories.
      This interpretation is required in the rest of the specification.
...
\paragraph{Comparison to prior specifications}
\begin{enumerate}
  \item Dybjer and Setzer's finite axiomatization \cite{TODO}. There are two main differences.
    First, Dybjer and Setzer factor out the $O$ field from the $\iota$ constructor into a separate
    annotation on signatures, called ``$\msf{Arg}$''. However, the bundle of a signature with an
    $\msf{Arg}$ over it is clearly equivalent to our version. Second, we have a countable universe
    hierarchy, while Dybjer and Setzer use a logical framework presentation with only three
    universes, $\msf{set}$, $\msf{stype}$ and $\msf{type}$, where $\msf{set}$ contains the
  ...
  \item The specification of small induction-recursion by Hancock et al. \cite{TODO}. Small IR mentions
    only two universes, $\Set$ and $\Set_1$, where the inductive type and the recursive output type
    are both in $\Set$, and $\Set_1$ hosts the type of signatures. Small IR signatures store an $O$
    value in the $\iota$ constructor, just like out signatures.
\end{enumerate}
