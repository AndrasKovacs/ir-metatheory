...
because Agda and Idris only have second-class IIR signatures, which cannot be neutral. There,
signatures consists of constructors, where constructors are disambiguated are canonical names (or
``tags''). $\El$ applied to a constructor computes definitionally, and so does the elimination
...
\item In $S$, we immediately pattern match on a tag.
\item All other $\Sig$ subterms are canonical in the rest of the signature.
\end{itemize}
In the Agda supplement, we provide length-indexed vectors and the $\Code$ universe as examples
for constructed IIR types with strict computation rules.
...
%% \begin{enumerate}
%%   \item $\El$ applied to a constructor
...
%% \begin{itemize}
%%   \item Every IIR signature in Agda has
...
%% (f (S0← (S0→ x)) (SIH← (mapIR P' (IR.elim P' f') (fst (S0→ x)))))


%% elim P f (intro x) = f x (map P (elim P f) x)

%% elim P f (intro x)
%% elim P f (intro (fst (S0→ x)), snd (S0→ x))
%% elimIR P' f' (intro (fst (S0→ x))) (snd (S0→ x))
%% f' (fst (S0→ x)) (map P' (elimIR P' f' (fst (S0→ x)))) (snd (S0→ x))
%% tr (λ(x, p). P (intro x, p)) (ε (S0→ x)) (f (S0← ((fst (S0→ x)), snd (S0→ x))) (SIH← (map P' (elimIR P' f' (fst (S0→ x))))))
%% tr (λ(x, p). P (intro x, p)) (ε (S0→ x)) (f (S0← (S0→ x)) (SIH← (mapIR P' (IR.elim P' f') (fst (S0→ x)))))


      %% let lhs = tr (λ {(x , p) → P (IR.wrap x , p)}) (F0rl S* (F0→ S* x))
      %%              (met (F0← S* (F0→ S* x)) (IH← S* (IR.mapIH S*' P' (IR.elim S*' P' met') (F0→ S* x .₁))))

%% f x (map P (elim P f) x)


%% f' x ih p = tr (λ(x, p). P (intro x, p)) (ε(x,p)) (f (S0← (x, p)) (SIH← ih))
