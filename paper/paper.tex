...
which is an instance of Definition \ref{TODO}. We omit describing the contextualization of
...
terms, we can look at an example for a canonicity statement. Recall $S$ from Example \ref{TODO} and
also the corresponding IIR predicate from Example \ref{TODO}. Assuming $t :
...
\end{alignat*}
Now, $\IIR_{(\floord{S^\w\,\ttt}\,\here)}\,t$ expresses that $t$ is either definitionally equal to $\intro\,(\Nat',\,\ttt)$
or to $\intro\,(\Pi',\,A,\,B,\,\ttt)$ for some canonical $A$ and $B$.
\end{example}
