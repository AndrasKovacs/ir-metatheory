...
\begin{alignat*}{4}
  ...
  & \intro^\w\,\{t\}\,t^\w := \intro_\IIR\,(\ora{(\Ssw)_{0^\w}}\,\,\here\,(t,\,\refl,\,t^\w))
\end{alignat*}
...
$S^\w$:
\[ \blank_{\floord{1}} : \forall\,S^\w\,p\,q\,t^\w.\,(\floord{S^\w}\,p)_1\,(\ora{(S^\w)_{0^\w}}\,p\,t^\w) = (\Ssw)_{1^\w}\,(\push_{0^\w}\,p\,q\,t^\w) \]
...
\begin{alignat*}{4}
  &k    &&:\,\,&&\Nat\\
  &P    &&:&& \Tm\,(\IR\,S^* \to \U_k)\\
  &P^\w &&:&& \{t : \Tm\,(\IR\,S^*)\} \to \IR^\w\,t \to \Tm\,(P\,t) \to \Set_k
\end{alignat*}
...
\begin{alignat*}{3}
  &\blank_{\IH^\w}  &&:\,\,&& (S^\w : \Sig^\w\,S)\to (S^\w)_{0^\w}\,t \to \Tm\,(S_\IH\,t) \to \Set_{\max(i,\,k)}\\
  &\blank_{\map^\w} &&:\,\,&& (S^\w : \Sig^\w\,S)(t^\w : (S^\w)_{0^\w}\,t)\{f : \Tm\,((x : \IR\,S^*) \to P\,x)\}\\
  &               &&  \,\,&&(f^\w : \{t : \Tm\,(\IR\,S^*)\}(t^\w : (S^\w)_{0^\w}\,t) \to P^\w\,t^\w\,(f\,t))\\
  &               &&      &&\to (S^\w)_{\IH^\w}\,S^\w\,t^\w\,(S_\map\,f\,t)
\end{alignat*}

 %%   mapIHᴾ : ∀{S}(Sᴾ : Sigᴾ S){x}(xᴾ : F0ᴾ Sᴾ x){f}(fᴾ : ∀ {x} xᴾ → Pᴾ xᴾ (f x)) → IHᴾ Sᴾ xᴾ (mapIH S f x)

  %% &f    &&:&& \Tm\,((t : (S^*)_0\,(\IR\,S^*)\,\El) \to (S^*)_\IH\,t \to P\,(\intro\,t))\\
  %% &f^\w &&:&& \{t : \Tm\,((S^*)_0\,(\IR\,S^*)\,\El)\}(t^\w : (\Ssw)_{0^\w}\,t)\{\ih : \Tm\,((S^*)_\IH\,t) \}\\
  %% &    &&  &&\to (\Ssw)_{\IH^\w}\,t^\w\,\ih \to P^\w\,(\intro^\w\,t^\w)\,(f\,t\,\ih)

%% We define the predicate interpretations for $\blank_\IH$ and $\blank_\map$ first, again
%% specializing the $\ir$ and $\el$ arguments to $\IR\,S$ and $\El$.
%% \begin{alignat*}{3}
%%   &\blank_{\IH^\w} : (S^\w : \Sig^\w\,S) \to (S^\w)_{0^\w}\,t \to \Tm\,(S_\IH\,t) \to \Set_{\max(i,\,k)}
%% \end{alignat*}

%% We aim to interpret $\elim$, which amounts to showing that $\elim$ preserves $\blank^\w$.
%% \[ \elim^\w : \{t : \Tm\,(\IR\,S^*)\}(t^\w : \IR^\w\,t \to) \to P^\w\,t^\w\,(\elim\,S^*\,P\,
