...
\begin{example} We compute the translation of the length-indexed vector signature from Example \ref{TODO}.
\begin{alignat*}{4}
...
\end{alignat*}
Here the first component $\Nat$ of the recursive result serves as the index. In the $\Cons'$ case we
have a single inductive field whose length is enforced with the extra $\sigma\,((x : \top) \to
\fst\,(f\,(\lup\!x)) = n)$.

\end{example}

\subsection{Type and Term Formers} Since the encoding of signatures
...
\begin{definition}[Interpretation of $\Elintro$]
...
isomorphism. For the second argument of $\floord{f^\w}$, like in Section \ref{TODO}, we show that
...
interpretation \cite{TODO}. In other words, we conjecture that our IR interpretation can serve as a
...
of $\blank^\w$ in Section \ref{TODO}, we need
\begin{alignat*}{4}
...
definition, and thus holds trivially.

Every other IR rule and substitution rule is interpreted similarly, as constant functions and
trivial equations. Also, $\Elintro$, and $\elimbeta$ are dispatched by direct appeal to the
equations that we have already proved. Hence, we omit these proofs here.

\todo{Let's have just one equation example which justifies putting the Pi and Sigma parametricity
  interpretations in the section}

This concludes the
canonicity interpretation for the object theory.
