...
the argument is indeed $n$. Finally, the length of the $\Cons'$ constructor is $\suc\,n$. For
...
\begin{alignat*}{3}
  ...
  &\quad \nil  \hspace{0.75em}: \Vect\,A\,0\\
  &\quad \cons : (n : \Nat) \to A \to \Vect\,A\,n \to \Vect\,A\,(\suc\,n)
\end{alignat*}
...
\begin{example}\label{ex:vector-constructors} Let us obtain the $\Vect$ constructors from the IIR rules. We take $S$ from Example \ref{ex:vector}. We define $\Vect\,A\,n : \U_0$ to be $\IIR\,S\,n$, where we appropriately instantiate $A$ in $S$. The derived constructors are the following.
  \begin{alignat*}{3}
    &\nil : \Vect\,A\,0                                  && \cons : (n : \Nat) \to A \to \Vect\,A\,n \to \Vect\,A\,(\suc\,n)\\
...
has been widely used to represent inductive families, e.g.\ in observational type theories
...
\todo{Fording of vector in Agda notation as well}
...
\todo{Explanation for the need of fixed and varying signatures}
...
\todo{illustration of paths and sub-signatures}

If we have a path to $S^\w : \Sig^\w\,S$, we can push the terms contained in the path onto a
term of $\E\,S\,(\IR\,S^*)\,\El$, getting a term of $\E\,S^*\,(\IR\,S^*)\,\El$.
\begin{alignat*}{4}
...
\todo{illustration of the restriction of functions in paths}

\begin{definition}[Encoding for signatures] We encode signatures by induction on $\Sig^\w\,S$;
...
This time around, encoded signatures directly yield the interpretation of $\IR$ and $\El$, and we do not need to
