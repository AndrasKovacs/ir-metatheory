...
Instances of inductive-recursive (IR) definitions were first used by Martin-Löf \cite{martin1975intuitionistic,martinlof84sambin}. General
definitions of IR types were formalized by Dybjer \cite{DBLP:journals/jsyml/Dybjer00} and Dybjer \&
Setzer \cite{dybjer99finite,DBLP:journals/jlp/DybjerS06,DBLP:journals/apal/DybjerS03}, who also
developed set-theoretic, realizability and categorical semantics. A common application of IR is to
define custom universe hierarchies inside a type theory. In the proof assistant Agda, we can use IR
to define a universe that is closed under our choice of type formers:
\begin{alignat*}{3}
  & \mbf{mutual} \\
...
Our signatures are identical to Dybjer and Setzer's \cite{DBLP:journals/apal/DybjerS03}, except for
...
We again follow Dybjer and Setzer \cite{DBLP:journals/apal/DybjerS03}. We assume a universe level
...
as a category with families (CwF) \cite{cwfs} where types are additionally annotated with levels.
Concretely, we have
\begin{itemize}
...
It is a proof-relevant logical predicate interpretation, which corresponds to \emph{Artin gluing}
\cite[Exposé~4,~Section~9.5]{SGA4}, i.e.\ categorical gluing over the global sections functor. The
concrete formulation that we use is the type-theoretic gluing by Kaposi, Huber and Sattler
\cite{gluing}, which is the same as Coquand's version \cite{coquand2018canonicity} when gluing along
the global sections functor. This is a variation of gluing which uses dependent type families
...
\begin{enumerate}
\item We first give semantic interpretation of object constructions while only referring to closed terms.
...
of interpretation functions on the object syntax. In phase (2) we do overload $\blank^\w$ and but
