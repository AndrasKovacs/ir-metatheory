...
that it does not generate too much confusion.
...
Consider the Agda code example in Section \ref{sec:introduction}. We present the canonicity
predicate for the IR type as a ``native'' metatheoretical IIR definition, focusing on readability.
First, the specification of the object-theoretic IR type, disregarding the eliminator:
\begin{alignat*}{4}
...
constructions. More concretely, we will do most constructions by induction on \emph{canonical
sub-signatures} of $S^*$.
...
\[  \floord{\E}\,\{S^\w\}\,p\,t \simeq \E\,\floord{S^\w}\,\IR^\w\,\El^\w \]
We write $\ora{\E}\,p$ for the map with type $\floord{\E}\,\{S^\w\}\,p\,t \to
\E\,\floord{S^\w}\,\IR^\w\,\El^\w$ and $\ola{\E}\,p$ for its inverse. This lets us interpret
$\intro$.
\begin{alignat*}{4}
...
\begin{alignat*}{3}
    &\Elintro^\w : \{t : \Tm\,(\E\,S^*\,(\IR\,S^*)\,\El)\}(t^\w : \E^\w\,S^{*\w}\,t) \to \El^\w\,(\intro^\w\,t^\w) = \F^\w\,t^\w\\
...
\begin{alignat*}{3}
  ...
  &f^\w &&:\,\,&& \{t\}(t^\w : \E^\w\,S^{*\w}\,t)\{\ih\} \to (\Ssw)_{\IH^\w}\,t^\w\,\ih \to P^\w\,(\intro^\w\,t^\w)\,(f\,t\,\ih)
\end{alignat*}
...
\begin{alignat*}{4}
  ...
  &\floord{P^\w}\,\{t\}\,t^\w := P^\w\,x^\w\,(\elim\,P\,f\,t).
\end{alignat*}
...
%% \begin{alignat*}{4}
%%   & \floord{f^\w}\,&&\Big(\ora{\E}\,\,\here\,(t,\,\refl,\,t^\w)\Big)\,\Big(\PSbe_\map\,\floord{P^\w}\,\elim^\w\,(\ora{\E}\,\,\here\,(t,\,\refl,\,t^\w)\Big)
%% \end{alignat*}
The left hand side computes to the following:
\begin{alignat*}{4}
  &f^\w\,&&\Big(\snd\,\big(\snd\,\big(\ola{(S^{*\w})_0}\,\here\,(\ora{\E}\,\,\here\,(t,\,\refl,\,t^\w))\big)\big)\Big)\\
  &      &&\Big(\ola{(S^{*\w})_{\IH}}\,\,\here\,\Big(\PSbe_\map\,\floord{P^\w}\,\elim^\w\,(\ora{\E}\,\,\here\,(t,\,\refl,\,t^\w))\Big)\Big)
\end{alignat*}
...
%% However, we conjecture that it is possible to get rid of UIP. In that case, the above
