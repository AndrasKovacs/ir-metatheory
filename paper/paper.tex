...
term is a numeral. Hence, canonicity justifies evaluation of closed terms. The outline of our
development is as follows.

\begin{itemize}
\item In Section \ref{sec:specification} we specify what it means to support IR and IIR, using
  Dybjer and Setzer's rules with minor modifications \cite{indexedir,DBLP:journals/apal/DybjerS03}. We use
...
objects. For the sake of readability, we split this action to two functions. $\E$ yields
the ``total type'' part of the interpretation of a signature, and $\F$ yields the morphism
part, i.e.\ a function into $O$.
\begin{alignat*}{3}
...
In IIR signatures, the sole deviation from \citet{DBLP:journals/jlp/DybjerS06} is again our use of
countable universe levels.
...
\end{itemize}
In short, the main idea is to represent IIR signatures as IR signatures together with a
...
\begin{alignat*}{3}
  ...
  &\intro\,x :\equiv (\intro_\IR\,(\fst\,(\ora{\E}\,x)),\,\snd\,(\ora{\E}\,x)) \quad\quad&& \msf{El\!\!-\!\!intro} && :\equiv \floord{\F}
\end{alignat*}
...
In the interpretation for types, note that $i$ appears in $\Ty\,\Gamma\,i$ as a natural number, but
it gets implicitly converted to a metatheoretic universe level in $\Set_i$. We will keep omitting
the conversion.
...
appropriately commutes with encoding and decoding.

\emph{Remark.} In the Agda formalization of $\elimbeta$, we use uniqueness of identity proofs
(UIP) instead of trying to shuffle transports by homotopical reasoning. We do this mainly because it
is easier. However, we conjecture that is possible skip UIP, and that might be useful in future applications
and variations of our construction; see Section \ref{TODO} for more discussion of this.
...
\end{alignat*}
We have previously defined a semantic $\IIR^\w$ with the following type (including all parameters):
\begin{alignat*}{4}
...
%% This is clearly not a fully precise embedding, because we can do more with arbitrary sets.
...
interpretations for its rules. It amounts to roughly 250 lines.
...
additional safety features degrade the ergonomics of working in Agda. Concretely, we need to check
...
because the sizes of $\blank^\w$ components are computed from syntactic levels in any case.
