...
\end{alignat*}
We give an informal explanation for why we need these paths. Consider
...
types, so we assume evident $\blank^\w$ interpretations for them. Now, $S^* : \Tm\,(\Sig_0\,\U_0)$
is a closed term that does not refer to any IR type or term former, so we can already fully compute
the action of the $\blank^\w$ operation on it:
\begin{alignat*}{3}
...
\end{alignat*}
Above, $\blank\$^\w\blank$ is the evident predicate interpretation of the $\blank\$\blank$
combinator. Then, we compute the following.
\begin{alignat*}{4}
