...
evident predicate interpretation of the $\blank\$\blank$ combinator. Now, $S^* :
\Tm\,(\Sig_0\,\U_0)$ is a closed term that does not refer to any IR type or term former, so we can
already fully compute the action of the $\blank^\w$ operation on it:
\begin{alignat*}{3}
...
metatheoretic operation on the object syntax and as such it computes on every syntactic
construction. Next, we compute the following.
\begin{alignat*}{4}
...
with some extra noise in the first argument of $\Pi'$, which is now represented as a function with
domain $\top$. We do not get stuck on neutral tags here either, but for a different reason: we have
function extensionality and equality reflection in the metatheory, so we are allowed to push
computation under the $t^\w$ split, up to meta-level definitional equality.
\end{example}
