...
%% We fix notation for IIR types.
%% \begin{itemize}
%% \item Recall from Section \ref{TODO} that IIR signatures are parameterized by $I$ and $O$.
%%       In the following, we set $I$ to be $\Tm\,(\IR\,S^*)$ and $O$ to be $O^\w \circ \El$.
%%       We write $\Sig_\IIR : \Set_j$ for the type of IIR signatures, parameterized by the
%%       specified $I$ and $O$.
%% \item We shall overload the $\Sig_\IIR$ constructors and other IIR operations, i.e.\
%%       we use $\iota$, $\sigma$, $\delta$ and so on.
%% \end{itemize}
As we
...
\begin{alignat*}{4}
  ...
  &\rlap{$\El^\w\,\Nat'^\w = \Nat^\w$}  \\
  &\rlap{$\El^\w\,(\Pi'^\w\,A^\w\,B^\w)\,f = \{a : \Tm\,A\} \to \El^\w\,A^\w\,a \to \El^\w\,B^\w\,(f\,a)$}
\end{alignat*}


%% \begin{alignat*}{4}
%%   &\Code^\w &&:\,&& \Tm\,\Code \to \Set_0\quad && \El^\w : \{t : \Tm\,\Code\} \to \Code^\w\,t \to (\Tm\,(\El\,t) \to \Set_0)\\
%%   &\Nat'^\w &&:\,&& \Code^\w\,\Nat' && \El^\w\,\Nat'^\w = \Nat^\w\\
%% \end{alignat*}
%% \begin{alignat*}{4}
%%   &\Pi'^\w  :\{A : \Tm\,\Code\}(A^\w : \Code^\w\,A)\{B : \Tm\,(\El\,A \to \Code)\}\\
%%   & \hspace{2.25em} (B^\w : \{a : \Tm\,(\El\,a)\} \to \El^\w\,A^\w\,a \to \Code^\w\,(B\,a)) \to \Code^\w\,(\Pi'\,A\,B)\\
%%   & \El^\w\,(\Pi'^\w\,A^\w\,B^\w)\,f = \{a : \Tm\,A\} \to A^\w\,a \to B^\w\,(f\,a)
%% \end{alignat*}
...
%% Let $S^*$ be the signature in Example \ref{TODO}. We present the encoded signature
%% $\floord{S^*}\,\here$ in an informal but hopefully readable way.




%%   we look at a logical predicate
%% interpretation of $\IR\,S^*$, using Agda-like IIsyntax to give




%%   It depends on a $\Tag$ type and $\Nat$, so we
%% shall assume $\blank^\w$ interpretations for them. Now, $S^* : \Tm\,(\Sig_0\,\U_0)$ is closed term
%% that does not refer to any IR type/term former, so we can already fully compute the $\blank^\w$
%% operation on it, to get $\Ssw : \Sig^\w\,(\lambda\,A.\,\Tm\,A \to \Set_0)\,S^*$. Then, we compute $\floord{\Ssw}\,\here$:
%% \begin{alignat*}{4}
...
%%   & \quad \Pi'^\w \to \sigma\,(\Tm\,(\top \to \IR\,S^*))\,\$\,\lambda\,A.\,
%%                       \delta\,((a : \Tm\,\top) \times \top)\,(\lambda\,(a,\,\_).\,A\,a)\,\$\,\lambda\,A^\w.\\
%%   & \hspace{4em} \sigma\,(\Tm\,(\top \to \IR\,S^*))\,\$\,\lambda\,f.\,
%%                       \delta\,((a : \Tm\,\top) \times \top)\,(\lambda\,(a,\,\_).\,f\,a)\,\$\,\lambda\,f^\w.
%% \end{alignat*}

  %% \sigma\,(\Tm\,\top)\,\$\,\lambda\,t.\,\sigma\,\top\,\$\,\lambda\,t^\w.\,\sigma\,(\Tm\,(\top \to \IR\,S^*))\,\$\,\lambda\,f.\\
  %% & \hspace{4em} \delta\,((a : \Tm\,\top) \times \top)\,(\lambda\,(a,\,\_).\,f\,a)\,\$\,\lambda\,f^\w.\\
