...
module ShallowIRTranslation5 (ext : Level) (ol : Level) (O : Set ol) (Oᴾ : O → Set ol) where
  ...
  module _ {S* : IR.Sig}(S*ᴾ : Sigᴾ S*) where
    ...
    F0ᴾ→ (δ Aᴾ Sᴾ) hom ((f , x) , eq , fᴾ , xᴾ) .₂ .₁ (a , aᴾ) = fᴾ aᴾ
    F0ᴾ→ (δ Aᴾ Sᴾ) hom ((f , x) , eq , fᴾ , xᴾ) .₂ .₂          = F0ᴾ→ (Sᴾ _) (δ< hom _ _) (x , eq , xᴾ)
    ...
    F0ᴾlr (δ Aᴾ Sᴾ) hom ((f , x) , eq , fᴾ , xᴾ) =
      ap (λ x → (f , x .₁) , x .₂ .₁ , (λ {x} → fᴾ {x}) , x .₂ .₂) -- implicit insertion shenanigans
         (F0ᴾlr (Sᴾ (Elᴾ ∘ fᴾ)) (δ< hom f (Elᴾ ∘ fᴾ)) (x , eq , xᴾ))
