...
module TranslationIR-IIR {ext il ol}(I : Set il) (O : I → Set ol) where
  ...
  F0-tr (IR.δ A x) u u' el el' i (fst , f0) f eq = (λ x₁ → f (fst x₁)) , F0-tr (x (λ x₁ → el' (f (fst x₁)))) u u' el el' i
                                                   (tr (λ p → IR.F0 (x p) (u i) el) (funext (λ z → el (fst z)) (λ z → el' (f (fst z))) (λ x₁ → eq)) f0) f eq
  ...
  F1-tr (IR.δ A Γ) {el = el} {el' = el'} i (f , t) f' eq f1 = F1-tr {!!} i {!!} {!!} {!!} {!!}
  ...
  mutual
    ...
    wrap {i} {Γ} (f0 , snd) = IR.wrap (F0-tr Γ (U Γ) (λ x → IR.U Γ) (λ {i'} x → i' , El Γ x) (IR.El Γ) i f0 (λ z → z .₁) f)
                              , {!!}
            where
              f : {x : U Γ i} → (i , El Γ x) ≡ IR.El Γ (x .₁)
              f {fst , refl} = refl
    -- one should be easy

  {-# TERMINATING #-}
  elim : ∀ {j}(Γ : Sig)(P : ∀ {i} → U Γ i → Set j) → (∀ {i} t → IH Γ (U Γ) (El Γ) P {i} t → P (wrap t)) → ∀ {i} t → P {i} t
  elim Γ P f {i} (f0 , eq) = (IR.elim Γ (λ x → ((i : I) → (eq : (IR.El Γ x .₁ ≡ i)) → P (x , eq))) {!!} f0) i eq
